...
const DataTicketCategoryPage = () => {
  ...
  useEffect(() => {
    ...
    dispatch(GetTicketCategoryData(params_attributes));
  }, [dispatch, meta]);
  ...
  const handleView = (data: any) => {
    ...
    navigate(`/ticket_category/view/${data.uuid}?${back}`);
  };

  const handleSearch = (search: string) => {
    setMeta({ ...meta, ticket_category_uuid: "", search });
    if (meta.page !== 1) {
  ...
  const handleCreate = () => {
    const back = `back=/ticket_category`;
    navigate(`/ticket_category/create?${back}`);
  };
...
export default DataTicketCategoryPage;
