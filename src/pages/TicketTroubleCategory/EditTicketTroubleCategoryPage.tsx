...
const EditTicketTroubleCategoryPage = () => {
  const { id } = useParams();
  ...
  useEffect(() => {
    ...
    if (messageUpdate !== "" && isSuccess && !isLoading) {
      ...
      navigate(`/ticket_trouble_category/view/${id}?${back}`);
      dispatch(resetTicketTroubleCategory());
  ...
  const handleSubmit = (e: any) => {
    e.preventDefault();
    dispatch(UpdateTicketTroubleCategoryData({ formData, uuid: id }));
  };
...
export default EditTicketTroubleCategoryPage;
