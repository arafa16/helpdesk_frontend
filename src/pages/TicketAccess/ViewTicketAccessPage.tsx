...
const ViewTicketAccessPage = () => {
  ...
  const handleEdit = () => {
    ...
    const back_view = `back_view=/ticket_access/view/${id}&back=${link_back}`;
    navigate(`/ticket_access/edit/${id}?` + back_view);
  };

  const handleDelete = () => {
    if (id) {
      if (window.confirm("Are you sure want to delete this data?")) {
        dispatch(DeleteTicketAccessData({ uuid: id }));
      }
    }
  };
...
export default ViewTicketAccessPage;
