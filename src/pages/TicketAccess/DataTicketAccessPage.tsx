...
const DataTicketAccessPage = () => {
  ...
  useEffect(() => {
    ...
    dispatch(GetTicketAccessData(params_attributes));
  }, [dispatch, meta]);
  ...
  const handleView = (data: any) => {
    ...
    navigate(`/ticket_access/view/${data.uuid}?${back}`);
  };

  const handleSearch = (search: string) => {
    setMeta({ ...meta, ticket_access_uuid: "", search });
    if (meta.page !== 1) {
  ...
  const handleCreate = () => {
    const back = `back=/ticket_access`;
    navigate(`/ticket_access/create?${back}`);
  };
...
export default DataTicketAccessPage;
