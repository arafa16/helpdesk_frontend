import Lucide from "../../base-components/Lucide";
import clsx from "clsx";
import Button from "../../base-components/Button";

const UserReminderTable = (props: any) => {
  const { datas, handleDelete, handleShowCreateSlideOver } = props;

  return (
    <div className="grid grid-cols-12 mt-5 box text-xs">
      {/* BEGIN: Inbox Content */}
      <div className="col-span-12 xl:col-span-12 2xl:col-span-12">
        <div className="pt-4 px-4">
          <p className="text-[12px]">User Riminder</p>
        </div>
        <div className="flex flex-col-reverse px-5 pb-2 border-b sm:flex-row text-slate-500 border-slate-200/60">
          <div className="flex items-center justify-between sm:ml-auto">
            <div className="my-2 sm:ml-auto">
              <Button
                size="sm"
                variant="primary"
                className="gap-4"
                onClick={() => handleShowCreateSlideOver()}
              >
                <Lucide icon="FilePlus" className="w-4 h-4" /> Add User
              </Button>
            </div>
          </div>
        </div>
        <div className="overflow-x-auto sm:overflow-x-visible px-4">
          {datas &&
            datas.map((data: any, index: any) => (
              <div key={index} className="intro-y my-4">
                <div
                  className={clsx([
                    "bg-slate-50 hover:bg-slate-100 rounded",
                    "transition duration-200 ease-in-out transform cursor-pointer inline-block sm:block border-b border-slate-200/60 dark:border-darkmode-400",
                  ])}
                >
                  <div className="flex items-center px-5 py-2 bg-slate-50 hover:bg-slate-100 rounded-lg">
                    <div className="mr-5 w-8">{index + 1}</div>
                    <div className="w-64 truncate sm:w-96">
                      <span className={clsx(["ml-3 truncate"])}>
                        {data?.user?.name}
                      </span>
                    </div>
                    <div className="w-64 truncate sm:w-96">
                      <span className={clsx(["ml-3 truncate"])}>
                        {data?.user?.email}
                      </span>
                    </div>
                    <div className="w-64 truncate sm:w-96">
                      <span className={clsx(["ml-3 truncate"])}>
                        {data.user?.phone_number}
                      </span>
                    </div>
                    <div
                      className="p-1 rounded-lg hover:bg-red-600 hover:text-white"
                      onClick={() => handleDelete(data.uuid)}
                    >
                      <Lucide icon="Trash2" className="w-4 h-4" />
                    </div>
                  </div>
                </div>
              </div>
            ))}
        </div>
      </div>
      {/* END: Inbox Content */}
    </div>
  );
};

export default UserReminderTable;
