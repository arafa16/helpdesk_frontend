  ...
  let box_status = (
    <div className="col-span-12 mt-6">
      <div className="md:flex md:justify-between">
        {status_first.map((status: any, index: number) => (
          <div key={index}>
            <div
              className={`px-5 py-3 mb-2 md:mb-0 box zoom-in w-full md:w-[150px] 2xl:w-[220px] truncate bg-slate-100`}
            >
              <div className="">{status?.name}</div>
              <div className="mt-3 text-xs font-medium">{status?.total}</div>
            </div>
          </div>
        ))}
      </div>
